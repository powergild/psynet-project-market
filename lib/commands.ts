...
// 행위형만 — "등록된/등록한"(=이미 등록됨=목록) 오인 방지 위해 "등록할/등록하/등록해"만.
const REGISTER_KEYS = [
  "등록할", "등록하", "등록해", "올릴래", "올릴게", "올리고", "구인할", "구인해", "구인하",
  "모집할", "모집해", "모집하", "사람 구", "사람구", "만들 사람", "만들사람",
  "새 프로젝트", "새프로젝트", "프로젝트 만들", "프로젝트 낼", "프로젝트 열", "프로젝트 개설",
];
...
function nlToCommand(
  ...
  if (["찾아줘", "찾아", "추천", "매칭", "할만한", "있어", "있나", "있는", "뭐가 있", "프로젝트", "목록", "리스트", "뭐 있", "뭐있", "보여"].some((k) => text.includes(k))) {
    // 먼저 여러 글자 군더더기 구를 제거한 뒤, 토큰 단위로 필러·한글자 토큰을 걸러 실제 검색어만 남긴다.
    const phraseStop = ["나와 맞는", "나한테 맞는", "뭐가 있어", "뭐 있어", "할만한거"];
    let remain = text;
    for (const s of phraseStop) remain = remain.split(s).join(" ");
    const FILLER = new Set([
      "찾아줘", "찾아", "추천", "추천해줘", "매칭", "매칭해줘", "할만한", "해줘", "해봐", "알려줘", "보여줘", "보여", "줘",
      "뭐가있어", "뭐있어", "뭐가", "뭐야", "뭐", "무슨", "어떤", "뭔가", "뭘", "무엇",
      "있어", "있나", "있는지", "있는", "없어", "목록", "리스트", "전체", "전부", "모든", "모두", "죄다",
      "지금", "현재", "요즘", "좀", "관련된", "관련", "분야", "쪽으로", "쪽", "등의", "등",
      "등록된", "등록한", "올라온", "올라와", "나와", "나한테", "맞는", "적합한", "어울리는", "괜찮은",
      "프로젝트", "프로젝트들", "프로젝트가", "프로젝트는", "프로젝트를", "거", "것", "게",
    ]);
    const tokens = remain
      .replace(/[?!._,]/g, " ")
      .split(/\s+/)
      .filter((t) => t.length >= 2 && !FILLER.has(t));
    // 남은 의미 토큰이 없으면(=의문사/목록성 질문) 전체 목록.
    return { command: tokens.length ? `매칭 ${tokens.join(" ")}` : "매칭", lastProjectId };
  }
