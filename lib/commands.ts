...
function nlToCommand(
  ...
  if (["찾아줘", "추천", "매칭", "할만한", "있어", "있나", "있는", "뭐가 있", "프로젝트", "목록", "리스트", "뭐 있", "뭐있"].some((k) => text.includes(k))) {
    const stop = [
      // 동작/요청어
      "찾아줘", "찾아", "추천해줘", "추천", "매칭해줘", "매칭", "할만한거", "할만한", "해줘", "해봐", "알려줘", "보여줘", "보여", "줘",
      // 의문사·목록성 (핵심 수정: 검색어로 오인 방지)
      "뭐가있어", "뭐가 있어", "뭐있어", "뭐 있어", "뭐가", "뭐야", "뭐", "무슨", "어떤", "뭔가", "뭘", "무엇",
      "있어?", "있어", "있나", "있는지", "있는", "없어?", "목록", "리스트", "전체", "전부", "모든", "모두", "죄다",
      // 시점/군더더기
      "지금", "현재", "요즘", "좀", "?", "!", ".", "관련된", "관련", "분야", "쪽으로", "쪽", "등의", "등",
      "나와 맞는", "나한테 맞는", "나한테", "나와", "맞는", "적합한", "어울리는", "괜찮은",
      "프로젝트", "프로젝트들", "프로젝트가", "프로젝트는", "프로젝트를",
    ];
    let remain = text;
    for (const s of stop) remain = remain.split(s).join(" ");
    remain = remain.replace(/[?!.]/g, " ").replace(/\s+/g, " ").trim();
    // 남은 의미 토큰이 없으면(=의문사/목록성 질문) 전체 목록.
    return { command: remain.length >= 2 ? `매칭 ${remain}` : "매칭", lastProjectId };
  }
